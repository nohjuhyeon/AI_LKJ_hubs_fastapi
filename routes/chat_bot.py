def reco_tour(region,theme,datediff):
    ...
    같은 날에 비슷한 테마의 관광지는 겹치면 안되고, 관광지 간의 이동하는 거리가 최소한으로 해줘.
    관광지는 중복되면 안돼
    이때 '관광지 : 도로명 주소 : 설명', 형태로 대답해주고, 각 관광지는 반드시 comma(,)로 분리해서 대답해주고, 날짜별로 줄바꿈해줘. 이때 중복되는 단어는 제외해줘. 
    이외의 말은 하지 마.
    같은 날에 관광지 간의 이동거리는 최소한으로 해줘. 이거는 꼭 지켜줘.
    아래와 같은 형식으로 대답해줘
    1일차
